...
# [WAJIB] Patch bawaan pyit2fls agar joblib.load tidak error
from pyit2fls import IT2Mamdani_ML, IT2Mamdani, IT2TSK, IT2TSK_ML, IT2FS_Gaussian_UncertMean, Optimizer
for attr in dir(IT2Mamdani):
    if attr.startswith('_IT2Mamdani__'):
        setattr(IT2Mamdani, attr.replace('_IT2Mamdani__', '__'), getattr(IT2Mamdani, attr))
for attr in dir(IT2Mamdani_ML):
    if attr.startswith('_IT2Mamdani_ML__'):
        setattr(IT2Mamdani_ML, attr.replace('_IT2Mamdani_ML__', '__'), getattr(IT2Mamdani_ML, attr))
for attr in dir(IT2TSK):
    if attr.startswith('_IT2Mamdani__'):
        setattr(IT2TSK, attr.replace('_IT2Mamdani__', '__'), getattr(IT2TSK, attr))
for attr in dir(IT2TSK_ML):
    if attr.startswith('_IT2Mamdani_ML__'):
        setattr(IT2TSK_ML, attr.replace('_IT2Mamdani_ML__', '__'), getattr(IT2TSK_ML, attr))
...
csv_file = open('Collected Data/1000paraNeuroIT2FS-.csv', 'w', newline='')
...
# ==========================================================
# ML PREDICTION SETUP  (cocok dengan trainMAMDANIT2_v2.py)
# ==========================================================
print("Memuat model IT2Mamdani_ML...")
ARTIFACT_DIR = Path("trained_artifacts_v2")
MODEL_PATH = ARTIFACT_DIR / "it2mamdani_pso_model.joblib"
META_PATH = ARTIFACT_DIR / "it2mamdani_pso_metadata.json"

trained_model = joblib.load(MODEL_PATH)

# Wajib baca metadata: training v2 simpan scaling sebenarnya di sini.
# Deploy lama menebak (deg+180)/360 -> SALAH. Sekarang pakai lo/hi asli.
with open(META_PATH, "r", encoding="utf-8") as fmeta:
    META = json.load(fmeta)

# Input scaler: mean +/- 3*std dengan padding 5%. lo/hi list panjang 2 (e, de).
X_LO = np.asarray(META["input_scaling"]["lo"], dtype=float)   # [e_lo, de_lo]
X_HI = np.asarray(META["input_scaling"]["hi"], dtype=float)   # [e_hi, de_hi]

# Output scaler: min-max [0,1] dengan padding 5%. lo/hi skalar.
Y_LO = float(META["output_scaling"]["lo"])
Y_HI = float(META["output_scaling"]["hi"])

# Sanity: input training pakai DERAJAT (e_deg_avg, de_deg_avg).
print(f"[SCALE] input lo={X_LO}  hi={X_HI}")
print(f"[SCALE] output lo={Y_LO:.6f}  hi={Y_HI:.6f}")
print(f"[INFO]  m_rules={META.get('m_rules')}  test R2={META.get('metrics',{}).get('test',{}).get('r2')}")


def _scale_input(x_deg):
    """Skala input ke [0,1] persis seperti make_input_scaler di training."""
    return np.clip((x_deg - X_LO) / (X_HI - X_LO), 0.0, 1.0)


def _unscale_output(y_scaled):
    """Balik output dari [0,1] ke skala mu asli."""
    return y_scaled * (Y_HI - Y_LO) + Y_LO


def predict_mu(e_rad, de_rad):
    # Training pakai derajat. Konversi dulu.
    ...
    # NaN/Inf guard: kalau model meleleh, jangan kirim sampah ke motor.
    if not math.isfinite(y_scaled):
        y_scaled = 0.0
    y_scaled = min(max(y_scaled, 0.0), 1.0)

    mu = _unscale_output(y_scaled)
    return float(mu)
...
class SerCOM:
    def __init__(self, port='COM6', baudrate=115200):
        ...
        # --- STATE MACHINE ---
        # State: 'IDLE', 'STANDBY' (Motor ON, diam), 'RUNNING' (Trajectory jalan)
        self.motor_state = 'IDLE'

    def calculate_edot(self, prev_error, current_error, delta_t):
        return (current_error - prev_error) / delta_t
    ...
    def read_from_arduino(self):
        ...
        while True:
            try:
                ...
                # Prediksi mu dari Machine Learning Model
                mu1 = predict_mu(e1, edot1)
                mu2 = predict_mu(e2, edot2)
                mu3 = predict_mu(e3, edot3)
                ...
                if self.motor_state == 'IDLE' or self.motor_state == 'STANDBY':
                    ...
                    data_to_send = f"{mu1:.4f},{mu2:.4f},{mu3:.4f},{self.setangle1:.4f},{self.setangle2:.4f},{self.setangle3:.4f}\n"
        ...
        while True:
            ...
            if command == 'S':
                ...
                if robot.motor_state == 'STANDBY':
                    ...
                    print("-> [R] Ditekan: Trajectory berjalan menggunakan ML. Merekam ke CSV...")
                else:
