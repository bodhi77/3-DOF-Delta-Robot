...
with PURE SYNERGETIC control (STATIC MU).

No serial, no Arduino, NO FUZZY LOGIC. 
The entire plant + control law is simulated in Python.
  - mu per-joint is set to a constant static value.
=====================================================================
"""
import math
import csv
...
MU1         = 0.003727       # Nilai mu statis konstan untuk ketiga motor
MU2         = 0.006000
MU3         = 0.003980

OUT_DIR           = str(_root_dir / "results" / "controller_sim" / "static_mu")
OUT_FIG           = os.path.join(OUT_DIR, "sim_helix_static_mu.png")
OUT_CSV           = os.path.join(OUT_DIR, "500helixSynergetic-SIM.csv")
...
def main():
    ...
    for step in range(n_steps):
        ...
        # --- mu is strictly STATIC (No Fuzzy) ---
...
def plot_results(arr):
    ...
    ax.set_title("mu (Static)"); ax.legend(fontsize=8); ax.grid(alpha=.3)
    ...
    fig.suptitle("Helix Simulation - Pure Synergetic (Static Mu)", fontweight='bold')
    fig.tight_layout()
    ...
    ax_indiv.set_title("Mu (Static)")
    ax_indiv.legend(fontsize=8)
    ax_indiv.grid(alpha=.3)
    fig_indiv.savefig(os.path.join(RESULT_DIR, "mu_static.png"))
    plt.close(fig_indiv)
