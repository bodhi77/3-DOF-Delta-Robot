...
# Filter lonjakan torsi (opsi 2): kandidat mu dengan |Delta-tau| (perubahan
# torsi langkah pertama vs step sebelumnya) di atas DTAU_MAX ditolak,
# supaya tidak ada lonjakan torsi kasar. Label mu* tetap dipilih dari error.
# Naikkan kalau terlalu banyak tertolak; turunkan kalau torsi masih kasar.
DTAU_MAX = 5.0
C_MANIFOLD = 80.0
...
for i in range(N_TOTAL):
    ...
    # fallback: lacak kandidat dengan Delta-tau terkecil kalau semua tertolak
    fb_dtau = float('inf')
    fb_mu = candidate_mu[0]
    fb_state_next = state.copy()
    fb_tau = np.zeros(3)
    ...
    for mu_test in candidate_mu:
        ...
        # akumulasi error tracking. mu terbaik = total error horizon terkecil.
        # Hanya langkah pertama yang nanti di-commit ke state utama.
        h_state = state.copy()
        h_cost = 0.0
        ...
        for h in range(HORIZON):
            ...
            h_cost += np.mean(np.abs(np.degrees(qd_next - s_next[0:3])))
            if h == 0:
        ...
        # --- FILTER LONJAKAN TORSI (opsi 2) ---
        # Delta-tau langkah pertama terhadap torsi step sebelumnya.
        # Buang kandidat dengan lonjakan > DTAU_MAX. Label mu* tetap
        # dipilih dari error saja (tidak teracak), kandidat berbahaya disaring.
        dtau = float(np.max(np.abs(first_tau - prev_tau_vec)))

        # lacak fallback (Delta-tau terkecil) untuk jaga-jaga semua tertolak
        if dtau < fb_dtau:
            fb_dtau = dtau
            fb_mu = mu_test
            fb_state_next = first_state_next
            fb_tau = first_tau

        if dtau > DTAU_MAX:
            continue                     # kandidat ditolak (lonjakan terlalu besar)
    ...
    # fallback: kalau semua kandidat tertolak, pakai yang Delta-tau terkecil
    if not np.isfinite(best_cost):
        best_mu = fb_mu
        best_state_next = fb_state_next
        best_tau = fb_tau
