...
with TRADITIONAL FUZZY SYNERGETIC control (IT2FLS).
...
in Python (ported from FuzzySynergeticDeltaRobotFullModelRev.ino).
  - mu per-joint is evaluated using traditional MyFuzzyT2 (pyit2fls).
=====================================================================
"""
import math
import csv
...
OUT_DIR           = str(_root_dir / "results" / "controller_sim" / "traditional_it2fs")
OUT_FIG           = os.path.join(OUT_DIR, "sim_helix_traditional_it2fs.png")
OUT_CSV           = os.path.join(OUT_DIR, "500helixFuzzySynergeticT2-SIM.csv")
...
# =====================================================================
# TRADITIONAL FUZZY MODEL (MyFuzzyT2) - ported from FuzzySynergeticT2.py
# =====================================================================
...
def predict_mu(e_val, edot_val):
    """Evaluasi sistem fuzzy IT2 tradisional untuk setiap joint"""
...
def main():
    ...
    for step in range(n_steps):
        ...
        # --- mu from Traditional IT2FLS (per joint) ---
...
def plot_results(arr):
    ...
    ax.set_title("mu (IT2FS)"); ax.legend(fontsize=8); ax.grid(alpha=.3)
    ...
    fig.suptitle("Helix Simulation - Traditional IT2 Fuzzy Synergetic", fontweight='bold')
    fig.tight_layout()
    ...
    ax_indiv.set_title("Mu (IT2FS)")
    ax_indiv.legend(fontsize=8)
    ax_indiv.grid(alpha=.3)
    fig_indiv.savefig(os.path.join(RESULT_DIR, "mu_it2fs.png"))
    plt.close(fig_indiv)
