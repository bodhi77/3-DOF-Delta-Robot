...
OUT_DIR           = str(_root_dir / "results" / "controller_sim" / "neuro_fuzzy_legacy")
OUT_CSV           = os.path.join(OUT_DIR, "1000helixNeuroIT2FS-SIM.csv")
OUT_FIG           = os.path.join(OUT_DIR, "sim_helix_neurofuzzy.png")
...
# =====================================================================
# HELIX TRAJECTORY (User's module - same as StaticMu / deploy)
# =====================================================================
...
M_PT      = 0.317 + PAYLOADS      # moving plate mass (kg)
...
def dynamics_matrices(t_now, q, dq, cache):
    """M_hat, C_hat, tau_g, J. Ported from steps 1-4 of computeSynergeticControl."""
    J, dJ = compute_jacobian(t_now, q, cache)
...
def main():
    ...
    for step in range(n_steps):
        ...
        # --- dynamics matrices (used for control & plant) ---
        J, M_hat, C_hat, tau_g = dynamics_matrices(t, q, dq, cache)

        # --- synergetic law ---
        sigma = c_manifold * e + ed_err
        acc_cmd = ddqd + c_manifold * ed_err + sigma / mu
        tau_ctrl = M_hat @ acc_cmd + C_hat @ dq + tau_g
        ...
        # --- mechanical plant: M_hat qdd + C_hat dq + tau_g = Kt*i ---
        rhs = Kt * i_curr - C_hat @ dq - tau_g
        try:
            qdd = np.linalg.solve(M_hat, rhs)
        except np.linalg.LinAlgError:
            qdd = np.zeros(3)
        dq = dq + qdd * DT
        q = q + dq * DT
...
def plot_results(arr):
    ...
    fig.suptitle("Helix Simulation - Neuro-Fuzzy Synergetic", fontweight='bold')
    fig.tight_layout()
