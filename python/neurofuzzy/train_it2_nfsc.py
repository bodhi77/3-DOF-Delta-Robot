...
"""
=====================================================================
IT2 Mamdani Neuro-Fuzzy Training (Revised - v2)
=====================================================================
...
_PLOT_DIR = _root_dir / "results" / "training_convergence" / "mamdani_t2"
...
CONFIG = {
    ...
    "model_filename": "it2mamdani_pso_model.joblib",
    "metadata_filename": "it2mamdani_pso_metadata.json",
}
...
def train_one(m_rules, X_train, y_train_s, X_val, y_val_s,
    ...
    """Train one IT2 Mamdani model. Returns (model, history dict)."""
    ...
    smoke = IT2Mamdani_ML(
        X_train.shape[1], m_rules,
        IT2FS_Gaussian_UncertStd, bounds,
        algorithm="PSO", algorithm_params=smoke_params,
    )
    n_smoke = min(smoke_samples, len(X_train))
    smoke_err, smoke_conv = smoke.fit(X_train[:n_smoke], y_train_s[:n_smoke])
    ...
    final_err, convergence = model.fit(X_train, y_train_s)
...
def main():
    ...
    fig.suptitle(
        f"IT2 Mamdani-PSO  |  rules={m_rules_used}{sweep_note}  |  "
        f"test RMSE={test_m['rmse']:.6f}  R^2={test_m['r2']:.4f}",
        fontsize=12, fontweight="bold", y=0.995,
    )
    plt.tight_layout(rect=[0, 0, 1, 0.97])
    plt.savefig(str(_PLOT_DIR / "mamdani_pso_result.png"), dpi=200)
    plt.savefig(str(_PLOT_DIR / "mamdani_pso_result.pdf"), format="pdf")
    ...
    plt.tight_layout()
    plt.savefig(str(_PLOT_DIR / "mamdani_T2_convergence.png"), dpi=200)
    plt.savefig(str(_PLOT_DIR / "mamdani_T2_convergence.pdf"), format="pdf")
    print("[SAVE] convergence plot -> mamdani_T2_convergence.png / .pdf")
    plt.show()
