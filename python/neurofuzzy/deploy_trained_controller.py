...
# Fleksibel: dukung T1 (T1Mamdani/T1Mamdani_ML) dan IT2 (IT2Mamdani/IT2Mamdani_ML).
...
def _unmangle(cls, tag):
    """Expose name-mangled private methods so joblib.load works."""
    pref = f"_{tag}__"
    for attr in dir(cls):
...
csv_file = open(str(COLLECTED_DATA_DIR / "1000helixNeuroMamdaniT1-1.csv"), 'w', newline='')
...
# ==========================================================
print("Memuat model fuzzy (T1 / IT2 - auto-detect)...")
ARTIFACT_DIR = TRAINED_ARTIFACTS_DIR

# Auto-detect: cari metadata yang ada (t1 atau it2). Bisa di-override manual.
_CANDIDATES = [
...
if MODEL_PATH is None:
    raise FileNotFoundError(
        f"Tidak menemukan model t1/it2 di {ARTIFACT_DIR}. "
        f"Pastikan ada *_pso_model.joblib + *_pso_metadata.json")
