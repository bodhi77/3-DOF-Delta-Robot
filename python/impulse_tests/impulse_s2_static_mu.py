"""
=====================================================================
OFFLINE SIMULATION: 3-DOF Delta Robot -- IMPULSE SCENARIO 2
(end-effector impulse force, Task 4) with PURE SYNERGETIC control (STATIC MU).

Auzan Task 4 / Scenario 2, 4-DOF -> 3-DOF. An unknown end-effector force
(0.5 N) is fired at t = 5 s, mapped to joint torques via J^T, while the robot
tracks a DYNAMIC trajectory (lissajousTraj). The synergetic law
must reject it. All settings live in the IMPULSE TEST block at the top of
CONFIGURATION.
...
# =====================================================================
#  Auzan Task 4 / Scenario 2: an unknown end-effector FORCE is fired at t = 5 s,
#  mapped to joint torques via J^T, while the robot tracks a DYNAMIC trajectory.
#  Kick too small -> raise S2_GAIN or widen IMPULSE_WIDTH.
S2_GAIN       = 60.0                         # strength (force scaled down by J^T -> needs a bigger number)
...
OUT_FIG           = os.path.join(OUT_DIR, "sim_impulse_s2_static.png")
...
# =====================================================================
# REFERENCE TRAJECTORY (dynamic path for Scenario 2)
# =====================================================================
from lissajousTraj import (
...
OUT_CSV = os.path.join(OUT_DIR, "impulse_s2_StaticMu-SIM.csv")
...
def main():
    ...
    print(f"[SIM] Scenario 2 (end-effector force)  impulse@{t_impulse:.3f}s  width={IMPULSE_WIDTH*1000:.0f}ms  "
          f"gain={S2_GAIN}  traj_time={traj_time:.3f}s  total_time={total_time:.3f}s  steps={n_steps}  dt={DT}s")
    ...
    for step in range(n_steps):
        ...
        if fired:
            ...
            f_ee = S2_GAIN * S2_FORCE * f_dir
            tau_dist_plant = J.T @ f_ee
        if fired and not impulse_done:
            impulse_done = True
            print(f"[IMPULSE] fired at t={t:.3f}s  scenario 2: {S2_GAIN*S2_FORCE:.3f} N EE force "
...
def plot_results(arr, t_impulse=None):
    ...
    fig.suptitle("Impulse S2 (end-effector force) - Pure Synergetic (Static Mu 1, 2, 3)", fontweight='bold')
    fig.tight_layout()
    ...
    for j in range(3):
        ...
        fig_indiv.savefig(os.path.join(RESULT_DIR, f"s2_joint_{j+1}_tracking.png"))
        plt.close(fig_indiv)
    ...
    fig_all.savefig(os.path.join(RESULT_DIR, "s2_joint_all_tracking.png"), dpi=130)
    plt.close(fig_all)
    ...
    ax_indiv.legend(fontsize=8)
    ax_indiv.grid(alpha=.3)
    fig_indiv.savefig(os.path.join(RESULT_DIR, "s2_mu_static.png"))
    plt.close(fig_indiv)
    ...
    ax_indiv.legend(fontsize=8)
    ax_indiv.grid(alpha=.3)
    fig_indiv.savefig(os.path.join(RESULT_DIR, "s2_position_error.png"))
    plt.close(fig_indiv)
    ...
    ax_indiv.legend(fontsize=8)
    ax_indiv.grid(alpha=.3)
    fig_indiv.savefig(os.path.join(RESULT_DIR, "s2_current_joint1.png"))
    plt.close(fig_indiv)
    ...
    ax_indiv.legend(fontsize=8)
    ax_indiv.grid(alpha=.3)
    fig_indiv.savefig(os.path.join(RESULT_DIR, "s2_xy_trajectory.png"))
    plt.close(fig_indiv)
    ...
    fig_indiv.savefig(os.path.join(RESULT_DIR, "s2_3d_trajectory.png"))
    plt.close(fig_indiv)
    ...
    ax_indiv.legend(fontsize=8)
    ax_indiv.grid(alpha=.3)
    fig_indiv.savefig(os.path.join(RESULT_DIR, "s2_z_over_time.png"))
    plt.close(fig_indiv)
