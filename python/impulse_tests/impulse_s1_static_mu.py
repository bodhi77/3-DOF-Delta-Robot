...
(control-torque impulse uncertainty) with PURE SYNERGETIC control (STATIC MU).
...
The synergetic law must reject it. All settings live in the IMPULSE
TEST block at the top of CONFIGURATION.

No serial, no Arduino, NO FUZZY LOGIC. The entire plant + control law is simulated
in Python (ported from FuzzySynergeticDeltaRobotFullModelRev.ino):
  ...
  - mu per-joint is set to 3 DIFFERENT constant static values (MU1, MU2, MU3)
...
# =====================================================================
# BIKIN KEK GINI: 3 NILAI MU STATIS BERBEDA UNTUK MASING-MASING MOTOR
# =====================================================================
MU1 = 0.003727
MU2 = 0.006000
MU3 = 0.003980
...
OUT_FIG           = os.path.join(OUT_DIR, "sim_impulse_s1_static.png")
OUT_CSV           = os.path.join(OUT_DIR, "impulse_s1_StaticMu-SIM.csv")
...
def forward_kinematics_dyn(q):
    """End-effector position (m) from joint angles q (rad)."""
    q1, q2, q3 = q
...
def main():
    ...
    t_impulse = S1_TIME
    total_time = max(traj_time, t_impulse + IMPULSE_WIDTH + RECOVERY_S)
    ...
    for step in range(n_steps):
        ...
        # ==============================================================
        # MENGGUNAKAN VARIABEL MU1, MU2, MU3 YANG ANDA TENTUKAN DI ATAS
        # ==============================================================
...
def plot_results(arr, t_impulse=None):
    ...
    ax.set_title("mu (Static 3 Vars)"); ax.legend(fontsize=8); ax.grid(alpha=.3)
    ...
    fig.suptitle("Impulse S1 (control-torque disturbance) - Pure Synergetic (Static Mu 1, 2, 3)", fontweight='bold')
    fig.tight_layout()
    fig.savefig(OUT_FIG, dpi=130)
    print(f"[OK] Combined graph saved: {OUT_FIG}")

    RESULT_DIR = str(_root_dir / "results" / "controller_sim" / "static_mu_impulse")
    ...
    ax_indiv.set_title("Mu (Static 3 Vars)")
    ax_indiv.legend(fontsize=8)
    ax_indiv.grid(alpha=.3)
    fig_indiv.savefig(os.path.join(RESULT_DIR, "s1_mu_static.png"))
    plt.close(fig_indiv)
