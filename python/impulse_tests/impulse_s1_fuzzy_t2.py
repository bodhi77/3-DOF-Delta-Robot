"""
=====================================================================
OFFLINE SIMULATION: 3-DOF Delta Robot -- IMPULSE SCENARIO 1
(control-torque impulse uncertainty) with TRADITIONAL FUZZY T2 SYNERGETIC control.

Auzan Scenario 1 (uncertainty rejection), 4-DOF -> 3-DOF. An impulse torque
disturbance [-1, -1, 1] N.m (adapted from Auzan's [-1, -1, 1, -1]) is applied to
the joints at t = 1 s while the robot HOLDS a static reference (stepTraj_static).
The synergetic / fuzzy law must reject it. All settings live in the IMPULSE
TEST block at the top of CONFIGURATION.
...
in Python (ported from FuzzySynergeticDeltaRobotFullModelRev.ino):
  ...
  - mu per-joint predicted by traditional Fuzzy T2 model
...
# =====================================================================
#  Auzan Scenario 1: an impulse TORQUE disturbance is applied straight onto the
#  joints at t = 1 s while the robot holds a STATIC reference (stepTraj_static).
#  Kick too small -> raise S1_GAIN or widen IMPULSE_WIDTH.
#  Robot leaves its workspace -> lower S1_GAIN.
S1_GAIN       = 3.0                              # strength (torque hits the plant directly; keep ~1-5)
S1_TIME       = 1.0                              # firing instant (s)
S1_TORQUE     = np.array([-1.0, -1.0, 1.0])      # N.m per joint (adapted from Auzan [-1,-1,1,-1])
IMPULSE_WIDTH = 0.02                             # pulse duration (s); bigger = more visible; 0 = single 1 ms step
RECOVERY_S    = 1.0                              # extra time kept AFTER the impulse to watch recovery (s)
# =====================================================================
...
OUT_DIR           = str(_root_dir / "results" / "controller_sim" / "impulse_combined")
OUT_FIG           = os.path.join(OUT_DIR, "sim_impulse_s1_fuzzyt2.png")
...
# =====================================================================
# REFERENCE TRAJECTORY (static hold for Scenario 1)
# =====================================================================
...
OUT_CSV = os.path.join(OUT_DIR, "impulse_s1_FuzzyT2-SIM.csv")

# =====================================================================
# TRADITIONAL FUZZY T2 MODEL (predict_mu) 
# =====================================================================
from fuzzy_t2_system import myfuzzyT2_sys
from pyit2fls import crisp
print("[INIT] Loading Traditional IT2FLS System...")
...
def predict_mu(e_val, edot_val, joint_idx):
    global last_mu
    try:
        _, tr = myIT2FLS.evaluate({"e": e_val, "edot": edot_val})
        mu_val = crisp(tr["mu"])
...
def main():
    ...
    print(f"[SIM] Scenario 1 (control-torque impulse)  impulse@{t_impulse:.3f}s  width={IMPULSE_WIDTH*1000:.0f}ms  "
          f"gain={S1_GAIN}  traj_time={traj_time:.3f}s  total_time={total_time:.3f}s  steps={n_steps}  dt={DT}s")
    ...
    for step in range(n_steps):
        ...
        tau_dist_plant = np.zeros(3)
        if fired:
            # Scenario 1: impulse torque disturbance applied DIRECTLY to the plant
            # (routing it through target_current lets the 12 V / current-loop
            # saturation swallow it). This is the torque the controller must reject.
            tau_dist_plant = S1_GAIN * S1_TORQUE
        if fired and not impulse_done:
            impulse_done = True
            print(f"[IMPULSE] fired at t={t:.3f}s  scenario 1: {S1_GAIN}*{S1_TORQUE} N.m "
...
def plot_results(arr, t_impulse=None):
    ...
    ax.set_title("mu (Fuzzy T2)"); ax.legend(fontsize=8); ax.grid(alpha=.3)
    ...
    fig.suptitle("Impulse S1 (control-torque disturbance) - Fuzzy T2 Synergetic", fontweight='bold')
    fig.tight_layout()
    fig.savefig(OUT_FIG, dpi=130)
    print(f"[OK] Combined graph saved: {OUT_FIG}")

    RESULT_DIR = str(_root_dir / "results" / "controller_sim" / "fuzzy_t2")
    ...
    for j in range(3):
        ...
        fig_indiv.savefig(os.path.join(RESULT_DIR, f"s1_joint_{j+1}_tracking.png"))
        plt.close(fig_indiv)
    ...
    fig_all.savefig(os.path.join(RESULT_DIR, "s1_joint_all_tracking.png"), dpi=130)
    plt.close(fig_all)

    fig_indiv, ax_indiv = plt.subplots()
    for j in range(3):
        ax_indiv.plot(t, arr[:, 7 + j], lw=1.2, label=f'mu{j+1}')
    ax_indiv.set_title("Mu (Fuzzy T2)")
    ax_indiv.legend(fontsize=8)
    ax_indiv.grid(alpha=.3)
    fig_indiv.savefig(os.path.join(RESULT_DIR, "s1_mu_fuzzyt2.png"))
    plt.close(fig_indiv)
    ...
    ax_indiv.legend(fontsize=8)
    ax_indiv.grid(alpha=.3)
    fig_indiv.savefig(os.path.join(RESULT_DIR, "s1_position_error.png"))
    plt.close(fig_indiv)
    ...
    ax_indiv.legend(fontsize=8)
    ax_indiv.grid(alpha=.3)
    fig_indiv.savefig(os.path.join(RESULT_DIR, "s1_current_joint1.png"))
    plt.close(fig_indiv)
    ...
    ax_indiv.legend(fontsize=8)
    ax_indiv.grid(alpha=.3)
    fig_indiv.savefig(os.path.join(RESULT_DIR, "s1_xy_trajectory.png"))
    plt.close(fig_indiv)
    ...
    fig_indiv.savefig(os.path.join(RESULT_DIR, "s1_3d_trajectory.png"))
    plt.close(fig_indiv)
    ...
    ax_indiv.legend(fontsize=8)
    ax_indiv.grid(alpha=.3)
    fig_indiv.savefig(os.path.join(RESULT_DIR, "s1_z_over_time.png"))
    plt.close(fig_indiv)
