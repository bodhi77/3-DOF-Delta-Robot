...
(end-effector impulse force, Task 4) with NEURO-FUZZY SYNERGETIC control.
...
tracks a DYNAMIC trajectory (lissajousTraj). The synergetic / neuro-fuzzy law
...
OUT_FIG           = os.path.join(OUT_DIR, "sim_impulse_s2_neurofuzzy.png")
...
OUT_CSV = os.path.join(OUT_DIR, "impulse_s2_NeuroMamdaniT1-SIM.csv")
...
def plot_results(arr, t_impulse=None):
    ...
    fig.suptitle("Impulse S2 (end-effector force) - Neuro-Fuzzy Synergetic", fontweight='bold')
    fig.tight_layout()
    ...
    ax_indiv.legend(fontsize=8)
    ax_indiv.grid(alpha=.3)
    fig_indiv.savefig(os.path.join(RESULT_DIR, "s2_mu_neurofuzzy.png"))
    plt.close(fig_indiv)
