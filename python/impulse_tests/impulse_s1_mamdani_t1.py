...
(control-torque impulse uncertainty) with TRADITIONAL FUZZY T1 SYNERGETIC control.
...
in Python (ported from FuzzySynergeticDeltaRobotFullModelRev.ino):
  ...
  - mu per-joint predicted by traditional Fuzzy T1 model
...
OUT_FIG           = os.path.join(OUT_DIR, "sim_impulse_s1_fuzzyt1.png")
...
OUT_CSV = os.path.join(OUT_DIR, "impulse_s1_FuzzyT1-SIM.csv")

# =====================================================================
# TRADITIONAL FUZZY T1 MODEL (predict_mu) 
# =====================================================================
...
def plot_results(arr, t_impulse=None):
    ...
    ax.set_title("mu (Fuzzy T1)"); ax.legend(fontsize=8); ax.grid(alpha=.3)
    ...
    fig.suptitle("Impulse S1 (control-torque disturbance) - Fuzzy T1 Synergetic", fontweight='bold')
    fig.tight_layout()
    fig.savefig(OUT_FIG, dpi=130)
    print(f"[OK] Combined graph saved: {OUT_FIG}")

    RESULT_DIR = str(_root_dir / "results" / "controller_sim" / "fuzzy_t1")
    ...
    ax_indiv.set_title("Mu (Fuzzy T1)")
    ax_indiv.legend(fontsize=8)
    ax_indiv.grid(alpha=.3)
    fig_indiv.savefig(os.path.join(RESULT_DIR, "s1_mu_fuzzyt1.png"))
    plt.close(fig_indiv)
