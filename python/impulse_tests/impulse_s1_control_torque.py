...
(control-torque impulse uncertainty) with NEURO-FUZZY SYNERGETIC control.
...
The synergetic / neuro-fuzzy law must reject it. All settings live in the IMPULSE
...
OUT_FIG           = os.path.join(OUT_DIR, "sim_impulse_s1_neurofuzzy.png")
...
OUT_CSV = os.path.join(OUT_DIR, "impulse_s1_NeuroMamdaniT1-SIM.csv")
...
def plot_results(arr, t_impulse=None):
    ...
    fig.suptitle("Impulse S1 (control-torque disturbance) - Neuro-Fuzzy Synergetic", fontweight='bold')
    fig.tight_layout()
    ...
    ax_indiv.legend(fontsize=8)
    ax_indiv.grid(alpha=.3)
    fig_indiv.savefig(os.path.join(RESULT_DIR, "s1_mu_neurofuzzy.png"))
    plt.close(fig_indiv)
