...
(end-effector impulse force, Task 4) with TRADITIONAL FUZZY T2 SYNERGETIC control.
...
tracks a DYNAMIC trajectory (lissajousTraj). The synergetic / fuzzy law
...
in Python (ported from FuzzySynergeticDeltaRobotFullModelRev.ino):
  ...
  - mu per-joint predicted by traditional Fuzzy T2 model (MyFuzzyT2)
...
OUT_FIG           = os.path.join(OUT_DIR, "sim_impulse_s2_fuzzyt2.png")
OUT_CSV           = os.path.join(OUT_DIR, "impulse_s2_FuzzyT2-SIM.csv")
...
def main():
    ...
    for step in range(n_steps):
        ...
        # ==============================================================
        # EVALUASI MU MENGGUNAKAN TRADITIONAL FUZZY T2
        # ==============================================================
...
def plot_results(arr, t_impulse=None):
    ...
    fig.suptitle("Impulse S2 (end-effector force) - Fuzzy T2 Synergetic", fontweight='bold')
    fig.tight_layout()
    ...
    ax_indiv.legend(fontsize=8)
    ax_indiv.grid(alpha=.3)
    fig_indiv.savefig(os.path.join(RESULT_DIR, "s2_mu_fuzzyt2.png"))
    plt.close(fig_indiv)
