...
(end-effector impulse force, Task 4) with TRADITIONAL FUZZY T1 SYNERGETIC control.
...
in Python (ported from FuzzySynergeticDeltaRobotFullModelRev.ino):
  ...
  - mu per-joint predicted by traditional Fuzzy T1 model (MyFuzzyT1)
...
OUT_DIR           = str(_root_dir / "results" / "controller_sim" / "fuzzy_t1")
OUT_FIG           = os.path.join(OUT_DIR, "sim_impulse_s2_fuzzyt1.png")
OUT_CSV           = os.path.join(OUT_DIR, "impulse_s2_FuzzyT1-SIM.csv")
...
def main():
    ...
    for step in range(n_steps):
        ...
        # ==============================================================
        # EVALUASI MU MENGGUNAKAN TRADITIONAL FUZZY T1
        # ==============================================================
...
def plot_results(arr, t_impulse=None):
    ...
    fig.suptitle("Impulse S2 (end-effector force) - Fuzzy T1 Synergetic", fontweight='bold')
    fig.tight_layout()
    ...
    ax_indiv.legend(fontsize=8)
    ax_indiv.grid(alpha=.3)
    fig_indiv.savefig(os.path.join(RESULT_DIR, "s2_mu_fuzzyt1.png"))
    plt.close(fig_indiv)
