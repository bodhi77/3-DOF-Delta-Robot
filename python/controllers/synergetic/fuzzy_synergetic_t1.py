...
csv_file = open(str(COLLECTED_DATA_DIR / "500helixFuzzySynergeticT1-10.csv"), 'w', newline='')
...
def fuzzy_m1(e1, edot1):
    global last_mu1
    _, crisp_outputs = myT1Mamdani.evaluate({"e": e1, "edot": edot1})
    mu1 = crisp_outputs["mu"] 

    if math.isnan(mu1): mu1 = last_mu1
    if not math.isnan(mu1): last_mu1 = mu1
    return mu1

def fuzzy_m2(e2, edot2):
    global last_mu2
    _, crisp_outputs = myT1Mamdani.evaluate({"e": e2, "edot": edot2})
    mu2 = crisp_outputs["mu"] 

    if math.isnan(mu2): mu2 = last_mu2
    if not math.isnan(mu2): last_mu2 = mu2
    return mu2

def fuzzy_m3(e3, edot3):
    global last_mu3
    _, crisp_outputs = myT1Mamdani.evaluate({"e": e3, "edot": edot3})
    mu3 = crisp_outputs["mu"] 

    if math.isnan(mu3): mu3 = last_mu3
    if not math.isnan(mu3): last_mu3 = mu3
    return mu3
    ...
    def read_from_arduino(self):
        ...
        while True:
            try:
                ...
                # Pengganti fuzzy: nilai mu statis
                mu1 = fuzzy_m1(e1, edot1)
                mu2 = fuzzy_m2(e2, edot2)
                mu3 = fuzzy_m3(e3, edot3)
