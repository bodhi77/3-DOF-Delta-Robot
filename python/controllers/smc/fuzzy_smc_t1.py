...
csv_file = open(str(COLLECTED_DATA_DIR / "500helixFuzzySMCT1-10.csv"), 'w', newline='')
...
def fuzzy_m1(e1, edot1):
    ...
    eta1 = crisp_outputs["eta"] 

    if math.isnan(eta1): eta1 = last_eta1
    if not math.isnan(eta1): last_eta1 = eta1
    return eta1

def fuzzy_m2(e2, edot2):
    global last_eta2
    _, crisp_outputs = myT1Mamdani.evaluate({"e": e2, "edot": edot2})
    eta2 = crisp_outputs["eta"] 

    if math.isnan(eta2): eta2 = last_eta2
    if not math.isnan(eta2): last_eta2 = eta2
    return eta2

def fuzzy_m3(e3, edot3):
    global last_eta3
    _, crisp_outputs = myT1Mamdani.evaluate({"e": e3, "edot": edot3})
    eta3 = crisp_outputs["eta"] 

    if math.isnan(eta3): eta3 = last_eta3
    if not math.isnan(eta3): last_eta3 = eta3
    return eta3
