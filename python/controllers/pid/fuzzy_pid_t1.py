...
csv_file = open(str(COLLECTED_DATA_DIR / "500helixFuzzyPIDT1-10.csv"), 'w', newline='')
...
# Serial setup (ubah 'COM6' ke port Arduino Anda)
...
def fuzzy_m1(e1, edot1):
    global last_Kp1, last_Ki1, last_Kd1
    _, tr = myT1Mamdani.evaluate({"e": e1, "edot": edot1})
    kpp1 = tr["kpp"]
    kdp1 = tr["kdp"]
    alpha1 = tr["alpha"]

    # Menghitung nilai PID berdasarkan rumus yang diberikan
    Kp1 = Kpmin + (Kpmax - Kpmin) * kpp1
    Kd1 = Kdmin + (Kdmax - Kdmin) * kdp1
    Ki1 = (kpp1** 2) / (kdp1 * alpha1) if kdp1 != 0 else 0

    # Jika hasilnya NaN, pakai nilai terakhir
    if math.isnan(Kp1): Kp1 = last_Kp1
    if math.isnan(Ki1): Ki1 = last_Ki1
    if math.isnan(Kd1): Kd1 = last_Kd1

    # Simpan nilai valid terbaru
    if not math.isnan(Kp1): last_Kp1 = Kp1
    if not math.isnan(Ki1): last_Ki1 = Ki1
    if not math.isnan(Kd1): last_Kd1 = Kd1

    return Kp1, Ki1, Kd1

def fuzzy_m2(e2, edot2):
    global last_Kp2, last_Ki2, last_Kd2
    _, tr = myT1Mamdani.evaluate({"e": e2, "edot": edot2})
    kpp2 = tr["kpp"]
    kdp2 = tr["kdp"]
    alpha2 = tr["alpha"]

    # Menghitung nilai PID berdasarkan rumus yang diberikan
    Kp2 = Kpmin + (Kpmax - Kpmin) * kpp2
    Kd2 = Kdmin + (Kdmax - Kdmin) * kdp2
    Ki2 = (kpp2** 2) / (kdp2 * alpha2) if kdp2 != 0 else 0

    # Jika hasilnya NaN, pakai nilai terakhir
    if math.isnan(Kp2): Kp2 = last_Kp2
    if math.isnan(Ki2): Ki2 = last_Ki2
    if math.isnan(Kd2): Kd2 = last_Kd2

    # Simpan nilai valid terbaru
    if not math.isnan(Kp2): last_Kp2 = Kp2
    if not math.isnan(Ki2): last_Ki2 = Ki2
    if not math.isnan(Kd2): last_Kd2 = Kd2

    return Kp2, Ki2, Kd2

def fuzzy_m3(e3, edot3):
    global last_Kp3, last_Ki3, last_Kd3
    _, tr = myT1Mamdani.evaluate({"e": e3, "edot": edot3})
    kpp3 = tr["kpp"]
    kdp3 = tr["kdp"]
    alpha3 = tr["alpha"]

    # Menghitung nilai PID berdasarkan rumus yang diberikan
    Kp3 = Kpmin + (Kpmax - Kpmin) * kpp3
    Kd3 = Kdmin + (Kdmax - Kdmin) * kdp3
    Ki3 = (kpp3** 2) / (kdp3 * alpha3) if kdp3 != 0 else 0

    # Jika hasilnya NaN, pakai nilai terakhir
    if math.isnan(Kp3): Kp3 = last_Kp3
    if math.isnan(Ki3): Ki3 = last_Ki3
    if math.isnan(Kd3): Kd3 = last_Kd3

    # Simpan nilai valid terbaru
    if not math.isnan(Kp3): last_Kp3 = Kp3
    if not math.isnan(Ki3): last_Ki3 = Ki3
    if not math.isnan(Kd3): last_Kd3 = Kd3
    ...
    def read_from_arduino(self):
        ...
        while True:
            try:
                ...
                Kp1, Ki1, Kd1 = fuzzy_m1(e1, edot1)
                # Kp2, Ki2, Kd2 = fuzzy_m2(e2, edot2)
                # Kp3, Ki3, Kd3 = fuzzy_m3(e3, edot3)
                ...
                if self.motor_state == 'IDLE' or self.motor_state == 'STANDBY':
                    ...
                    data_to_send = f"{Kp1:.4f},{Ki1:.4f},{Kd1:.4f},{self.setangle1:.4f},{self.setangle2:.4f},{self.setangle3:.4f}\n"
